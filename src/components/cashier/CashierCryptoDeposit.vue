    ...
    <div class="deposit-info">
      <div class="info-qrcode">
        <transition name="fade" mode="out-in">
          ...
          <div v-else class="qrcode-content" key="data">
            <QRCode
              v-bind:value="cashierCryptoData.addresses[modalsData.currency]"
      ...
      <div class="info-text">
        <div class="text-header">
          <img
            v-bind:src="
              require('@/assets/img/cashier/' + modalsData.currency + '.png')
            "
          />
          {{ cashierGetName }}
        </div>
        ...
        <p>
          ...
          <span>Minimum 0.01 {{ modalsData.currency.toUpperCase() }}.</span>
          Your deposit will be confirmed after
          {{ cashierGetConfirmations }} confirmation on the network.
        </p>
      </div>
    </div>
        ...
        <div class="content-element">
          <transition name="fade" mode="out-in">
            ...
            <div v-else class="element-content" key="data">
              ...
              <input
                v-model="cashierCoinAmount"
                v-on:input="modalCoinInput"
                type="text"
              />
            </div>
          </transition>
        ...
        <div class="content-element element-crypto">
          <transition name="fade" mode="out-in">
            ...
            <div v-else class="element-content" key="data">
              <img
                v-bind:src="
                  require('@/assets/img/cashier/' +
                    modalsData.currency +
                    '.png')
                "
              />
              <input
                v-model="cashierCryptoAmount"
                v-on:input="modalCryptoInput"
                type="text"
              />
            </div>
          </transition>
        ...
        <div class="content-element">
          <transition name="fade" mode="out-in">
            ...
            <div v-else class="element-content" key="data">
              ...
              <input
                v-model="cashierFiatAmount"
                v-on:input="modalFiatInput"
                type="text"
              />
            </div>
          </transition>
      ...
      <div class="address-input">
        <transition name="fade" mode="out-in">
          ...
          <div v-else class="input-content" key="data">
            <input
              v-model="cashierCryptoData.addresses[modalsData.currency]"
              type="text"
            />
...
export default {
  ...
  methods: {
    ...
    modalCoinInput() {
      this.cashierFiatAmount = parseFloat(
        (this.cashierCoinAmount / 1000) * 3
      ).toFixed(2);
      this.cashierCryptoAmount = parseFloat(
        (1 /
          (this.cashierCryptoData.prices[this.modalsData.currency].price /
            1000)) *
          this.cashierFiatAmount
      ).toFixed(8);
    },
    modalFiatInput() {
      this.cashierCoinAmount = parseFloat(
        (this.cashierFiatAmount / 3) * 1000
      ).toFixed(2);
      this.cashierCryptoAmount = parseFloat(
        (1 /
          (this.cashierCryptoData.prices[this.modalsData.currency].price /
    ...
    modalCryptoInput() {
      this.cashierFiatAmount = parseFloat(
        this.cashierCryptoAmount *
          (this.cashierCryptoData.prices[this.modalsData.currency].price / 1000)
      ).toFixed(2);
      this.cashierCoinAmount = parseFloat(
        (this.cashierFiatAmount / 3) * 1000
      ).toFixed(2);
  ...
  computed: {
    ...mapGetters(["modalsShow", "modalsData", "cashierCryptoData"]),
    cashierGetName() {
